...
public class WareSkuServiceImpl extends ServiceImpl<WareSkuDao, WareSkuEntity> implements WareSkuService {
    ...
    public Boolean orderLockStock(WareSkuLockTO lockTO) {
        ...
        // 3.查询（库存 - 库存锁定 >= 待锁定库存数）的仓库    WareSkuEntity: wms_ware_sku 中的一行数据（库存 - 库存锁定 >= 待锁定库存数）
        List<WareSkuEntity> wareEntities = baseMapper.selectListHasSkuStock(lockItemMap.keySet()).stream().
                filter(entity -> entity.getStock() - entity.getStockLocked() >= lockItemMap.get(entity.getSkuId()).getCount()).collect(Collectors.toList());
        // 判断是否查询到仓库
        ...
        if (wareMap.size() < lockTO.getLocks().size()) {
            ...
            for (Map.Entry<Long, List<WareSkuEntity>> entry : wareMap.entrySet()) {
                ...
                List<WareSkuEntity> hasStockWares = entry.getValue();// 有足够库存的仓库   //感觉多此一举，上面已经判断 存量足够
